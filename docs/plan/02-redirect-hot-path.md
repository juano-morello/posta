...
#### T2.3.1 · `feat: capture event dto in contracts` [INV-6][INV-8]
...
#### T2.3.2 · `feat: header extraction into the capture dto`
...
#### T2.3.6 · `feat: daily salt manager with atomic first-use generation`
...
#### T2.3.7 · `feat: visitor_hash and immediate ip discard` [INV-6][security]
The client IP is read once from `CF-Connecting-IP` (falling back to the leftmost `X-Forwarded-For` entry), passed to `lookupNetwork` and to `computeVisitorHash(ip, userAgent, salt)` = `sha256(ip + ua + salt).hex.slice(0, 32)`, and never leaves that function's scope. `buildCapturePayload()` takes the *derived* values — `visitor_hash`, `asn`, `country` — as parameters, so there is no call path along which an IP can reach the payload builder, the queue, or a log line. This is structural, not a convention: the type of the payload builder does not admit an IP.
