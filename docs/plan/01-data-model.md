...
#### T1.1.1 · `chore: postgres client and drizzle-kit config in packages/core` ✅ done (`73178f6`)
...
#### T1.1.2 · `test: testcontainers Postgres 16 harness for packages/core` ✅ done (`baa4ecf`)
...
#### T1.1.3 · `feat: ULID helper in packages/core` ✅ done (`b274b6b`)
...
#### T1.1.4 · `feat: users table aligned with Better Auth's expected shape` ✅ done (`b8bd1e0`)
...
#### T1.1.5 · `feat: links table with UNIQUE (tenant_id, slug)` ✅ done (`9421b69`)
...
#### T1.1.6 · `feat: bio_pages table` ✅ done (`c81691b`)
...
#### T1.1.7 · `feat: bio_links join table referencing links.id` ✅ done (`bc95271`)
...
#### T1.1.8 · `feat: domains table with unused type column as the v1.5 seam` ✅ done (`a57edbd`)
...
#### T1.1.9 · `feat: tenant-scoped repository helper in packages/core/src/db` ✅ done (`56d4448`)
...
#### T1.1.10 · `test: forbid direct tenant-table access outside the tenant helper` ✅ done (`4f99b3f`)
...
#### T1.1.11 · `feat: link DTOs in contracts with absolute-URL destination validation` [security] ✅ done (`f3d3ed4`)
...
#### T1.1.12 · `feat: bio DTOs in contracts with reserved-handle rejection` [security] ✅ done (`6d32d26`)
...
#### T1.2.1 · `feat: hand-written SQL migration runner in packages/core/src/db` ✅ done (`22cae1f`)
...
#### T1.2.2 · `feat: partitioned events table, hand-written SQL` ✅ done (`30da9d0`)
...
#### T1.2.3 · `feat: parent-level indexes for tenant+link event queries` ✅ done (`bdabbf9`)
...
#### T1.2.4 · `feat: read-only Drizzle typing for events` ✅ done (`d4f2f31`)
...
#### T1.2.5 · `test: assert events has no ip and no classification column` [INV-4][INV-6][security] ✅ done (`3e24be6`)
...
#### T1.2.6 · `feat: rollback support for hand-written SQL migrations` ✅ done (`553a33f`)
...
#### T1.3.1 · `feat: create_events_partition(month) SQL function` ✅ done (`2661ab8`)
...
#### T1.3.2 · `feat: DEFAULT partition for events` ✅ done (`68dd1e1`)
...
#### T1.3.3 · `feat: bootstrap current plus three months of partitions on migrate` ✅ done (`bc5ea3f`)
...
#### T1.3.4 · `feat: scheduled partition maintenance job in apps/worker` ✅ done (`edcd1ac`)
...
#### T1.3.5 · `feat: alert on non-empty events_default` ✅ done (`dd56dcb`)
...
#### T1.3.6 · `test: partition boundary and pruning tests` ✅ done (`b3dd065`)
...
#### T1.4.1 · `feat: asn_datacenter lookup table` ✅ done (`0410ef0`)
...
#### T1.4.2 · `chore: vendored datacenter ASN list with provenance` ✅ done (`08cb6a5`)
...
#### T1.4.3 · `feat: idempotent asn_datacenter seed script` ✅ done (`da6c66a`)
...
#### T1.4.4 · `docs: ASN list refresh procedure` ✅ done (`b9821a1`)
...
#### T1.5.1 · `feat: unified pnpm migrate running drizzle then hand-written SQL` ✅ done (`06fb543`)
...
#### T1.5.2 · `feat: pnpm migrate:status` ✅ done (`7be3a7d`)
...
#### T1.5.3 · `feat: pnpm migrate:down for the last hand-written migration` ✅ done (`aa4a011`)
...
#### T1.5.4 · `ci: run migrations from empty on every build` ✅ done (`456fcb6`)
...
#### T1.5.5 · `feat: pnpm seed creates the single v1 account` [INV-9][security] ✅ done (`17d73d4`)
...
#### T1.5.6 · `docs: why migrations come in two flavors` ✅ done (`435a343`)
