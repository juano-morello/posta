...
Zod create/update DTOs for links — the schema boundary the acceptance criterion refers to. `destination` must be an absolute `http(s)` URL: `javascript:`, `data:`, `file:`, protocol-relative `//host`, and bare relative paths are all rejected before anything reaches the database. `slug` is lowercase alnum + dash + underscore, 1–64 chars, and rejects the reserved paths `favicon.ico` and `robots.txt` from spec §3.1.
