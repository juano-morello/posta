...
| 2026-07-21 | `packageManager` carries the corepack integrity hash, written by `corepack use` | Hand-writing the hash from `npm view <pkg> dist.integrity` | Two different encodings share the name "integrity". npm's `dist.integrity` is SRI — `sha512-<base64>`, containing `/` and `=`. Corepack's `packageManager` suffix is `+sha512.<hex>`. Turbo parses this field against `\+[0-9A-Za-z.-]+`, so the SRI form breaks *every* turbo command with `invalid_package_manager_field` while the corepack form is fine. Always regenerate with `corepack use pnpm@<version>`; never paste `dist.integrity`. |
| 2026-07-21 | Guard core with a dependency-cruiser rule (static + dynamic import) | The React `server-only` package | `server-only`'s only non-react-server export is an unconditional throw; plain Node (api, worker) never sets that condition, so importing core would crash both services at boot. depcruise fails the build in CI instead, and catches dynamic imports too. Recorded so nobody re-adds `server-only`. |
| 2026-07-26 | **GHCR** as the container registry (`vars.REGISTRY` = `ghcr.io/<owner>`) | A cloud-provider registry (ECR / GAR / ACR) | Same-org registry, so Actions mints a `GITHUB_TOKEN` per run and there is no long-lived credential to store or rotate — needs only `packages: write`. Picking a provider registry would have dragged the compute provider along with it; images are pulled by digest from anywhere, so **compute stays unchosen**. Note: a package published from a public repo still starts **private** — that's an imagePullSecret until flipped. |
