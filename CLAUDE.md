...
| 2026-07-21 | `packageManager: pnpm@10.33.0` with **no** corepack integrity hash | `pnpm@10.33.0+sha512.<digest>` | Turbo 2.10.5 (latest) parses `packageManager` against `\+[0-9A-Za-z.-]+`, which cannot match a base64 sha512 — those always contain `/` and `=`. With the hash present, *every* turbo command fails `invalid_package_manager_field`, so this is forced, not preferred. Dependency integrity is unaffected: `pnpm-lock.yaml` carries a per-package sha512 and CI installs with `--frozen-lockfile`. Only the pnpm binary itself loses corepack verification — pin it by exact version in CI instead. Recorded here so nobody later "fixes" the missing hash and breaks the whole build. Revisit if turbo widens the regex. |
