...
// (T3.2.1's ua.ts), in-app browser detection (T3.2.2's source-platform.ts),
// and destination-host extraction (T3.2.4's dest-host.ts). Consumers
// (apps/worker, later apps/api) import from '@posta/core' or this module,
// never reach past it into './ua', './source-platform', or './dest-host'
// directly — same pattern as ../geoip/index.ts and ../redis/index.ts.
//
...
// isInApp, destHost) — no bot/human verdict field exists to leak
// (invariant 4), but a future addition to any file in this directory
